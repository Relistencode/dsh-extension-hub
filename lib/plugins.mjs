...
import { fileRows, writeFlatRows } from './loaderpatch.mjs'
...
function registeredSlugs(webProfileDir) {
  const slugs = new Set()
  let text = ''
  try {
  ...
  for (const match of text.matchAll(/^\s*name:\s*['"]?([^'"\s]+)['"]?\s*$/gm)) {
    const value = match[1]
    if (path.isAbsolute(value) && value.toLowerCase().startsWith(dir.toLowerCase() + path.sep.toLowerCase())) {
      slugs.add(path.basename(value).toLowerCase())
    }
  }
  // Also accept explicit row ids matching the clone slug.
  for (const match of text.matchAll(/^\s*- id:\s*(\S+)\s*$/gm)) slugs.add(match[1].toLowerCase())
  return slugs
}
 ...
 * The row is written as a flat top-level list entry; it takes effect on the
 * next dsh web start.
 */
export async function installPlugin(webProfileDir, repo) {
  ...
  const rows = fileRows(patchPath).filter((r) => !(r && r.id === slug))
  rows.push({ id: slug, name: target })
  writeFlatRows(patchPath, rows)
  // Verify the registration row actually landed in the patch file.
  let wrote = false
  try {
    wrote = new RegExp(`^\\s*- id:\\s*${slug}\\s*$`, 'm').test(fs.readFileSync(patchPath, 'utf8'))
  } catch {
    wrote = false
  }
  if (!wrote) {
