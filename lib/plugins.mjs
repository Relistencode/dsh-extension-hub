 ...
 * query). Marks repositories that are already installed locally.
 */
export async function discoverPlugins(webProfileDir, query) {
  const terms = `topic:dsh-plugin${query && String(query).trim() ? ` ${String(query).trim()}` : ''}`
  const url = `https://api.github.com/search/repositories?q=${encodeURIComponent(terms)}&sort=stars&order=desc&per_page=30`
  let res
  try {
    res = await fetch(url, {
  ...
  try {
    for (const dir of fs.readdirSync(pluginsDir())) installed.add(dir.toLowerCase())
  } catch {
    // no plugins dir yet
  }
...
export async function installPlugin(webProfileDir, repo) {
  ...
  upsertServer(path.join(webProfileDir, 'cordis.patch.yml'), { id: slug, name: target })
