...
export async function installPlugin(webProfileDir, repo) {
  ...
  if (!pkg) {
    ...
    return { ok: false, code: 'invalid', message: '仓库没有可用的 package.json，可能不是 dsh 插件包' }
  }
  if (typeof pkg.main !== 'string' || !fs.existsSync(path.join(target, pkg.main))) {
    fs.rmSync(target, { recursive: true, force: true })
    return {
      ok: false,
      code: 'invalid',
  ...
  upsertServer(patchPath, { id: slug, name: pluginRowName(target, pkg.main) })
...
export function featureCatalog() {
  return [
    ...
    { id: 'vision-toolkit', name: '@anionex/dsh-vision-toolkit' },
  ]
}
...
/**
 * Snapshot every add-on feature: installed (patch row present — in the
 * profile patch for Extension Hub installs, or in a bundle layer for
 * official `dsh plugin add` installs), enabled (no disabled override), and
 * the exact patch row id to act on.
 */
export function listFeatures(webProfileDir) {
  const patchPath = path.join(webProfileDir, 'cordis.patch.yml')
  const toggles = toggleMap(patchPath)
  const bundles = bundlePatches(webProfileDir)
  const features = featureCatalog().map((f) => {
    // Profile patch first (Extension Hub's own installs), then the bundle
    // layers (official `dsh plugin add` installs).
    let rowId = patchRowIdByPackageName(patchPath, f.name)
    let viaBundle = false
    if (rowId === null) {
      for (const { patchPath: bundlePatch } of bundles) {
        const id = patchRowIdByPackageName(bundlePatch, f.name)
        if (id !== null) {
          rowId = id
          viaBundle = true
          break
