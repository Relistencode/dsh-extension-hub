...
import { listPlugins as pluginSnapshot, removePluginRow, discoverPlugins, installPlugin, installNpmPlugin, pluginsDir, updateNpmPackage, checkPluginUpdates as pluginUpdates, updatePluginItem, listFeatures, toFilePath } from './plugins.mjs'
...
class extensionHubGateway extends TypertRemoteService {
  ...
  removePlugin(input) {
    ...
    if (typeof result.name === 'string' && result.name !== '') {
      // Clone rows store a `file://` URL (loader-importable on Windows);
      // convert it back to a path before touching the filesystem.
      const rawName = toFilePath(result.name)
