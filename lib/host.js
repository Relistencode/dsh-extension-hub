...
import { listPlugins as pluginSnapshot, removePluginRow, discoverPlugins, installPlugin, installNpmPlugin, pluginsDir, updateNpmPackage, checkPluginUpdates as pluginUpdates, updatePluginItem, listFeatures, cloneRoot } from './plugins.mjs'
import { curatedCatalog } from './catalog.mjs'
...
class extensionHubGateway extends TypertRemoteService {
  ...
  async importItems(input) {
    ...
    if (kind === 'mcp' || kind === 'all') {
      ...
      if (scope === 'project') {
        ...
        const r = await installMcpGlobal(entries)
  ...
  // managed region of the profile cordis.patch.yml (insert/override patch
  // semantics — a bare top-level row would mean "override" and silently no-op).
  ...
  // cordis.patch.yml. Built-in bundle rows are not present in the file, so
  // they report ok:false (disable them instead). When the row points at a
  // local clone under the managed plugins dir, the clone is removed too.
  // Takes effect on the next dsh web start.
  removePlugin(input) {
    input = input || {}
    const id = typeof input.id === 'string' ? input.id.trim() : ''
    ...
    if (typeof result.name === 'string' && result.name !== '') {
      // Clone rows store a `file://` URL (loader-importable on Windows) that
      // may point at the clone root or an entry file inside it — collapse to
      // the clone root before touching the filesystem.
      const rawName = cloneRoot(result.name)
      const dir = pluginsDir()
      const resolved = path.resolve(rawName)
      ...
      } else if (!path.isAbsolute(rawName) && !rawName.startsWith('@deepseek-ai/')) {
        // npm-installed third-party plugin: remove the package files too.
        const webDir = path.join(dshHome(), 'profiles', 'web')
        const pkgDir = path.join(webDir, 'node_modules', ...(rawName.startsWith('@') ? rawName.split('/') : [rawName]))
        if (fs.existsSync(pkgDir)) {
          try {
            fs.rmSync(pkgDir, { recursive: true, force: true })
            message += '；已删除 npm 包文件'
          } catch {
            // package removal failed; keep going
          }
        }
      }
    }
  ...
  // Plugin Management tab: snapshot every add-on feature (installed/enabled
  // state plus the patch row id to act on). See lib/plugins.mjs.
  listFeatures(input) {
    input = input || {}
    return jsonSafe(listFeatures(path.join(dshHome(), 'profiles', 'web')))
  }
  ...
  // Curated store tab: browse the community curated catalog with
  // category/order/query/page filtering (see lib/catalog.mjs).
  async curatedPlugins(input) {
    input = input || {}
    return jsonSafe(await curatedCatalog(path.join(dshHome(), 'profiles', 'web'), input))
  }

  // Install a plugin. npm installs (input.npm) are preferred: registry
  // tarball, anti-squatting check against the selected repo (input.fullName),
  // patch row registration. GitHub installs (input.repo) fall back to the
  // clone+patch path. Both take effect on the next dsh web start.
  async installPlugin(input) {
    input = input || {}
    const webDir = path.join(dshHome(), 'profiles', 'web')
    const npm = typeof input.npm === 'string' ? input.npm.trim() : ''
    if (npm !== '') {
      const fullName = typeof input.fullName === 'string' ? input.fullName.trim() : ''
      return jsonSafe(await installNpmPlugin(webDir, npm, fullName))
    }
    const repo = typeof input.repo === 'string' ? input.repo : ''
    return jsonSafe(await installPlugin(webDir, repo))
  }

  // Compare this plugin AND every installed add-on feature against the npm
  // registry. The result is unified on purpose: the UI shows one "update
  // available" state and updating refreshes everything at once.
  ...
  async checkUpdates(input) {
    ...
    try {
      ...
      let anyUpdate = latest !== current
      const installed = listFeatures(webDir).features.filter((f) => f.installed)
      for (const f of installed) {
        try {
          const localPkg = JSON.parse(fs.readFileSync(path.join(webDir, 'node_modules', f.name, 'package.json'), 'utf8'))
          const localVersion = typeof localPkg.version === 'string' ? localPkg.version : null
          const encoded = f.name.startsWith('@') ? f.name.replace('/', '%2F') : f.name
          const fres = await fetch(`https://registry.npmjs.org/${encoded}/latest`, { signal: AbortSignal.timeout(8000) })
          if (fres.ok) {
            const fmeta = await fres.json()
            const fLatest = fmeta && typeof fmeta.version === 'string' ? fmeta.version : null
            if (fLatest && fLatest !== localVersion) anyUpdate = true
          }
        } catch {
          // a feature check failure never downgrades the unified status
        }
      }
      return jsonSafe({ status: anyUpdate ? 'update-available' : 'up-to-date', current, latest })
    } catch (error) {
      return jsonSafe({ status: 'error', current, message: error && error.message ? error.message : String(error) })
    }
  }

  // Upgrade this plugin AND every installed add-on feature in the web profile
  // WITHOUT pnpm (tarball download, system tar, replace node_modules, update
  // the dependency declaration). A restart of dsh web is required for the new
  // code to take effect.
  async updatePlugin(input) {
    input = input || {}
    const webDir = path.join(dshHome(), 'profiles', 'web')
    const main = await updateNpmPackage(webDir, 'dsh-extension-hub')
    const installed = listFeatures(webDir).features.filter((f) => f.installed)
    let featureFailures = 0
    for (const f of installed) {
      try {
        const r = await updateNpmPackage(webDir, f.name)
        if (!(r && r.ok === true)) featureFailures++
      } catch {
        featureFailures++
      }
    }
    const result = {
      ok: main.ok === true && featureFailures === 0,
      version: main.version,
      partialFailure: main.ok === true && featureFailures > 0,
    }
    if (featureFailures > 0) {
      result.message = '部分功能更新失败，请稍后重试'
    }
