...
class extensionHubGateway extends TypertRemoteService {
  ...
  // Upgrade this plugin in the web profile via pnpm. Returns ok + the newly
  // installed version, or ok:false with the pnpm error message. A restart of
  ...
  async updatePlugin(input) {
    ...
    try {
      ...
      const { stdout } = await execFileAsync(bin, ['add', 'dsh-extension-hub@latest'], {
        cwd: webDir,
        timeout: 120000,
        windowsHide: true,
        maxBuffer: 1024 * 1024,
      ...
      let version = null
      try {
        const req = createRequire(path.join(webDir, 'probe.js'))
        const pkg = JSON.parse(fs.readFileSync(req.resolve('dsh-extension-hub/package.json'), 'utf8'))
        version = typeof pkg.version === 'string' ? pkg.version : null
      } catch {
        // version unreadable after update; still report success
      }
      return jsonSafe({ ok: true, version, output: (stdout || '').split('\n').slice(-8).join('\n') })
    } catch (error) {
