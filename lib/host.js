...
class extensionHubGateway extends TypertRemoteService {
  ...
  // Enable/disable a plugin row by writing a `disabled` flag into the profile
  // cordis.patch.yml (flat row format). Takes effect on the next dsh web start.
  setPluginEnabled(input) {
    ...
    const rows = fileRows(patchPath)
    const row = rows.find((r) => r && r.id === id)
    if (row) row.disabled = !input.enabled
    else rows.push({ id, disabled: !input.enabled })
    writeFlatRows(patchPath, rows)
    return jsonSafe({ ok: true, pending: true, id })
  }

  // Uninstall a plugin row by deleting it from the profile cordis.patch.yml.
  // Built-in bundle rows are not present in the patch file, so they report
  // ok:false (disable them instead). When the row points at a local clone
  // under the managed plugins dir, the clone is removed too. Takes effect on
  // the next dsh web start.
  removePlugin(input) {
    input = input || {}
    const id = typeof input.id === 'string' ? input.id.trim() : ''
    if (id === '') throw new Error('missing plugin id')
    ...
    writeFlatRows(patchPath, rows.filter((r) => !(r && r.id === id)))
    let message = '已从配置移除，重启 dsh web 后生效'
    if (typeof row.name === 'string' && row.name !== '') {
      const dir = pluginsDir()
      const resolved = path.resolve(row.name)
