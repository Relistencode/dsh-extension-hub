...
import { listPlugins as pluginSnapshot, removePluginRow, discoverPlugins, installPlugin, installNpmPlugin, pluginsDir, updateNpmPackage, checkPluginUpdates as pluginUpdates, updatePluginItem, listFeatures } from './plugins.mjs'
...
class extensionHubGateway extends TypertRemoteService {
  ...
  removePlugin(input) {
    ...
    if (typeof result.name === 'string' && result.name !== '') {
      ...
      } else if (!path.isAbsolute(result.name) && !result.name.startsWith('@deepseek-ai/')) {
        // npm-installed third-party plugin: remove the package files too.
        const webDir = path.join(dshHome(), 'profiles', 'web')
        const pkgDir = path.join(webDir, 'node_modules', ...(result.name.startsWith('@') ? result.name.split('/') : [result.name]))
