	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"" + CSS_ID + "\"]") === null) {
			...
			tag.textContent = [
				...
				".skm-list{display:flex;flex-direction:column;gap:4px;max-height:52vh;overflow-y:auto;padding-right:4px}",
				...
				".skm-group-head{display:flex;align-items:center;gap:8px;cursor:pointer;user-select:none;padding:4px 2px;border:none;background:none;font-family:inherit;font-size:13px;font-weight:600;color:var(--dsw-alias-label-primary,#1f2329);width:100%;text-align:left}",
				...
				".skm-plugin-row{cursor:pointer;width:100%;text-align:left;border:1px solid var(--dsw-alias-border,#e5e6eb);border-radius:10px;padding:6px 10px;background:var(--dsw-alias-bg-layer-1,rgba(0,0,0,.015));font-family:inherit}",
				...
				".skm-plugin-detail-row{display:flex;gap:8px;align-items:flex-start}",
				...
				".skm-plugin-name{font-weight:500;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:13px;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
			].join("");
		...
		var REMOTE_METHODS = ["list", "getSkill", "createSkill", "updateSkill", "removeSkill", "toggleSkill", "getMcp", "upsertMcp", "removeMcp", "toggleMcp", "discover", "importItems", "projectInfo", "getState", "setState", "pickProjectFolder", "listDirectory", "createDirectory", "listPlugins", "setPluginEnabled", "removePlugin", "discoverPlugins", "installPlugin", "checkPluginUpdates", "updatePluginItem", "checkUpdates", "updatePlugin"];
						...
						items.map(function (p) {
							...
							return h("div", { key: p.entryId, className: "skm-plugin-row" + (isSelected ? " skm-selected" : ""), onClick: function () { setSelected(isSelected ? null : p.entryId); } },
								h("div", { className: "skm-row" },
									...
									upd ? h("span", { style: { display: "inline-flex" } },
		...
		// ── Discover tab (GitHub dsh-plugin search + install) ────────────────────
		function DiscoverTab(props) {
			...
			var load = function (q) {
				...
				setRepos(null);
				setDetail(null);
				call("discoverPlugins", { query: q || "" }).then(function (r) {
					if (r && r.ok === false) {
			...
			var runInstall = function (repo) {
				...
				call("installPlugin", { repo: repo.fullName }).then(function (r) {
					...
					if (r && r.ok === false) {
						...
						setRepos(updated);
						setDetail(Object.assign({}, repo, { installed: true }));
					} else {
			...
			if (detail) {
				var r = detail;
				return h("div", { style: { display: "flex", flexDirection: "column", gap: 10 } },
					Btn({ sm: true, onClick: function () { setDetail(null); } }, t("discover.backToResults")),
					h("div", { className: "skm-item-title", style: { fontSize: 15 } }, r.fullName),
					r.description ? h("div", { className: "skm-desc" }, r.description) : null,
					h("div", { className: "skm-plugin-detail", style: { padding: 0 } },
						h("div", { className: "skm-plugin-detail-row" },
							h("b", null, t("discover.stars")),
							h("span", null, "⭐ " + String(r.stars || 0))),
						r.language ? h("div", { className: "skm-plugin-detail-row" },
							h("b", null, t("discover.language")),
							h("span", null, r.language)) : null,
						r.updatedAt ? h("div", { className: "skm-plugin-detail-row" },
							h("b", null, t("discover.updated")),
							h("span", null, String(r.updatedAt).slice(0, 10))) : null,
						r.htmlUrl ? h("div", { className: "skm-plugin-detail-row" },
							h("b", null, t("discover.repository")),
							h("a", { href: r.htmlUrl, target: "_blank", rel: "noopener noreferrer", style: { color: "var(--dsw-alias-accent,#3370ff)" } }, r.htmlUrl)) : null),
					h("div", { className: "skm-row", style: { gap: 8 } },
						r.installed
							...
							: Btn({ kind: "primary", onClick: function () { setConfirm(r); } }, t("discover.install"))),
