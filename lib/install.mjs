...
// Rows are written as a flat top-level list (loader-serialization compatible).

export function installMcpGlobal(entries) {
  const patch = hostPatchPath()
  const out = []
  for (const entry of entries) {
    const rows = fileRows(patch).filter((r) => !(r && r.id === entry.id))
    rows.push({ ...entry })
    writeFlatRows(patch, rows)
...
export function removeMcpGlobal(serverName) {
  ...
  writeFlatRows(patch, fileRows(patch).filter((r) => !(r && r.id === id)))
...
export function toggleMcpGlobal(serverName, disabled) {
  const patch = hostPatchPath()
  const id = serverName.startsWith('mcp-') ? serverName : `mcp-${serverName}`
  const rows = fileRows(patch)
  const row = rows.find((r) => r && r.id === id)
  if (row) row.disabled = !!disabled
  else rows.push({ id, disabled: !!disabled })
  writeFlatRows(patch, rows)
